...
class NutritionInfo(Base):
    ...
    recipe_id = Column(String(36), ForeignKey("recipes.recipe_id", ondelete="CASCADE"), unique=True, nullable=False, comment="食谱ID")
    calories = Column(Float, nullable=False, comment="卡路里")
    protein = Column(Float, nullable=False, comment="蛋白质(g)")
    carbohydrates = Column(Float, nullable=False, comment="碳水化合物(g)")
